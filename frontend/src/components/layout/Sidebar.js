...
const Sidebar = () => {
  const { user, logout, isAdmin, isSuperAdmin, canManageAttendance, switchSchool, joinSchool } = useAuth();
  ...
  const adminItems = [
    ...
    { to: '/admin/scholen', icon: School, label: 'Scholen', superadminOnly: true },
  ];
    ...
    <div className="flex flex-col h-full">
      ...
      {user?.activeSchoolName ? (
        <div className="mx-4 mb-2">
          <button
            onClick={() => setSchoolSwitcherOpen(!schoolSwitcherOpen)}
            className="w-full px-3 py-2 rounded-lg bg-white/10 backdrop-blur-sm hover:bg-white/15 transition-colors cursor-pointer"
          >
            <div className="flex items-center gap-2">
              <School className="w-3.5 h-3.5 text-white/70 flex-shrink-0" />
              <span className="text-xs font-medium text-white/90 truncate flex-1 text-left">{user.activeSchoolName}</span>
              <ChevronDown className={cn('w-3 h-3 text-white/50 transition-transform', schoolSwitcherOpen && 'rotate-180')} />
            </div>
          </button>
          {schoolSwitcherOpen && (
            <div className="mt-1 space-y-0.5 animate-slideUp">
              {user.schools?.filter(s => s.id !== user.activeSchoolId).map(s => (
                <button
                  key={s.id}
                  onClick={async () => {
                    try {
                      ...
                      toast.success(`Gewisseld naar ${s.name}`);
                      window.location.href = '/dashboard';
                    } catch (err) {
                      toast.error('Kon niet wisselen van school');
                    }
                  }}
                  className="w-full text-left px-3 py-1.5 text-xs text-white/70 hover:text-white hover:bg-white/10 rounded-md transition-colors"
                >
                  {s.name}
                </button>
              ))}
              ...
              <div className="pt-1 border-t border-white/10 mt-1">
                <p className="px-3 py-1 text-[10px] text-white/40">School toevoegen:</p>
                <div className="flex gap-1 px-2">
                  <input
                    ...
                    className="flex-1 px-2 py-1 text-xs bg-white/10 border border-white/20 rounded text-white placeholder-white/30 font-mono uppercase"
                  />
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        toast.error(err.response?.data?.detail || 'Ongeldige code');
                      } finally {
                        setJoining(false);
                      }
                    }}
                    className="px-2 py-1 text-xs bg-white/20 hover:bg-white/30 text-white rounded transition-colors disabled:opacity-50"
                  >
                    +
                  </button>
                </div>
        ...
        <div className="mx-4 mb-2">
          <div className="px-3 py-2 rounded-lg bg-amber-500/20">
            <p className="text-[10px] text-amber-200 mb-1">Geen school geselecteerd</p>
